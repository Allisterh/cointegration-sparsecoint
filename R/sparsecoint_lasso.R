...
initBetaFinal <- function (Y, Z, X, alpha.init, p, q, rank) {
  ...
  Yinit <- (Y - X %*% matrix(rbind(rep(diag(1, ncol(Y)), p)), ncol = ncol(Y), byrow = T)) %*% alpha.init
  for (i in 1:rank) {
...
initAlpha <- function (Y, Z, X, beta.init, p) {
  SVDinit <- svd(t(Z %*% beta.init) %*% (Y - X %*% matrix(rbind(rep(diag(1, ncol(Y)), p)), ncol = ncol(Y), byrow = T)))
